...
def main():
    ...
    df_corr = perform_EDA(df_nodupLE)
...
def perform_EDA(df):
    ...
    my_corr = EDA_Tools.correlation_matrix(df)


    # Select features whose correlation with target is > 0.2
    corr_columns = Feature_Selection_Tools.get_features_by_corr(df, target='DEPRESSED', corr_threshold=0.2)
...
def model_training(x_train, x_test, y_train, y_test):
    ...
    grid_search_dt = GridSearchCV(
        estimator=model, param_grid=parameters, scoring="accuracy", cv=5, verbose=1
    )
